...
#SBATCH -J 13_1_multiteacher_math
#SBATCH -N 1
...
GENERATION_SHARDS="${GENERATION_SHARDS:?GENERATION_SHARDS is required}"
LIMIT="${LIMIT:-1000}"
...
if [ "${GENERATION_SHARDS}" -le 0 ]; then
  echo "GENERATION_SHARDS must be positive." >&2
  exit 2
fi
...
select_stably_idle_gpus \
  "${MIN_FREE_MIB}" "${GENERATION_SHARDS}" "${WAIT_SECONDS}" \
  "${MAX_USED_MIB}" "${MAX_UTILIZATION}" "${STABLE_CHECKS}"
GPU_IDS=("${GPU_IDS[@]:0:${GENERATION_SHARDS}}")
if [ "${#GPU_IDS[@]}" -ne "${GENERATION_SHARDS}" ]; then
  echo "Idle-GPU selection returned ${#GPU_IDS[@]} GPUs, expected ${GENERATION_SHARDS}." >&2
  exit 2
fi
...
for shard_index in "${!GPU_IDS[@]}"; do
  ...
  log_path="${GENERATOR_OUTPUT}/worker_${shard_index}_of_${GENERATION_SHARDS}_gpu_${gpu_id}.log"
  (
    CUDA_VISIBLE_DEVICES="${gpu_id}" python3 -u scripts/5_1_generate_capacity_length_traces.py \
      --config "${CONFIG}" \
      --generator-name "${GENERATOR_NAME}" \
      --output-dir "${GENERATOR_OUTPUT}" \
      --num-shards "${GENERATION_SHARDS}" \
...
if [ "${failed}" -ne 0 ]; then
  echo "At least one ${GENERATOR_NAME} generation shard failed." >&2
  exit 1
fi
echo "multiteacher_math_generation_complete generator=${GENERATOR_NAME} shards=${GENERATION_SHARDS} gpus=${GPU_IDS[*]} output=${GENERATOR_OUTPUT}"
