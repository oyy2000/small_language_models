#!/usr/bin/env python3
"""Evaluate the base model and all 36 ranked main-matrix adapters."""
...
from length_budget_distill.ranked_multiteacher_evaluation import validated_matrix_training_runs
...
def main() -> None:
    ...
    tasks = [
        {
            ...
            "budget_name": run["budget_name"],
            "seed": run["seed"],
            "adapter_path": run["adapter_path"],
            "adapter_config_sha256": run["adapter_config_sha256"],
            "adapter_model_sha256": run["adapter_model_sha256"],
            "training_data_sha256": run["train_sha256"],
            "training_examples": run["n"],
            "supervised_tokens": run["supervised_tokens"],
        }
        for run in training_runs
    ]
    tasks.append(
        {
            "model_id": "base", "run_name": "base_qwen2p5_1p5b_instruct", "mode": "base",
            "generator_name": None, "budget_name": None, "seed": None, "adapter_path": None,
        }
    )
    ...
    for task in tasks:
        ...
        command = [
            sys.executable, "scripts/4_1_eval_model.py", "--config", str(config_path),
            "--model-name", str(config["student"]["model_name"]),
            "--split", str(evaluation["dataset_split"]),
            "--start-index", str(evaluation["start_index"]),
            "--limit", str(evaluation["limit"]),
            "--output-jsonl", entry["prediction_path"],
            "--summary-json", entry["summary_path"],
            "--max-new-tokens", str(evaluation["max_new_tokens"]),
            "--temperature", str(evaluation["temperature"]),
            "--top-p", str(evaluation["top_p"]),
            "--batch-size", str(evaluation["batch_size"]),
            "--torch-dtype", str(config["student"]["torch_dtype"]),
        ]
    ...
    manifest = {
        "status": "running",
        "stage": "ranked_multiteacher_main_matrix",
        ...
        "decoding": {key: evaluation[key] for key in ("max_new_tokens", "temperature", "top_p", "batch_size")},
        ...
        "validation_source_sha256": file_sha256(
            PROJECT_ROOT / "src/length_budget_distill/ranked_multiteacher_evaluation.py"
        ),
    ...
    if not gpu_ids or max_parallel <= 0 or max_parallel > len(gpu_ids):
        raise ValueError("Invalid GPU IDs or --max-parallel.")
    failures = _run_tasks(entries, commands, gpu_ids, max_parallel, evaluation, manifest, manifest_path, args.skip_complete)
...
def _run_tasks(
    entries: List[Dict[str, Any]], commands: List[List[str]], gpu_ids: List[str],
    max_parallel: int, evaluation: Dict[str, Any], manifest: Dict[str, Any],
    manifest_path: Path, skip_complete: bool,
) -> List[Dict[str, Any]]:
    ...
    while pending or running:
        while pending and len(running) < max_parallel and available:
            ...
            if existing:
                raise FileExistsError(f"Incomplete evaluation artifacts for {entry['model_id']}: {existing}")
            gpu = available.pop(0)
            env = os.environ.copy()
...
def _completed(entry: Mapping[str, Any], evaluation: Mapping[str, Any]) -> Dict[str, Any] | None:
    return completed_evaluation_evidence(
        entry["prediction_path"], entry["summary_path"], expected_n=int(evaluation["limit"]),
...
def _check_support(expected: List[str] | None, observed: List[str], model_id: Any) -> List[str]:
    if expected is not None and expected != observed:
        raise ValueError(f"Evaluation support mismatch for {model_id}")
