#!/usr/bin/env python3
"""Independently audit and seal the three ranked-length SFT runs."""
...
EXPECTED_RUN_NAMES = {
    ...
    "equal_example__qwen2p5_7b__relative_medium__seed_17",
    "equal_example__qwen2p5_7b__relative_long__seed_17",
}
...
def main() -> None:
    ...
    if report_path.exists() or marker_path.exists():
        raise FileExistsError(
            f"Refusing to overwrite ranked-length training audit evidence: {output_dir}"
        )
    ...
    _expect(
        ...
        "Launch manifest references a different prepared dataset manifest.",
        errors,
    )
    ...
    _expect(
        set(prepared_runs) == EXPECTED_RUN_NAMES,
        f"Prepared run set mismatch: {sorted(prepared_runs)}",
        errors,
    )
    _expect(
        set(launch_runs) == EXPECTED_RUN_NAMES,
        f"Launch run set mismatch: {sorted(launch_runs)}",
        errors,
    )
    _expect(
    ...
    for run_name in sorted(EXPECTED_RUN_NAMES):
        ...
        for key in (
            ...
            _expect(
                launch_run.get(key) == prepared_run.get(key),
        ...
        if evidence is None:
            errors.append(f"Adapter evidence is incomplete or invalid for {run_name}: {output_path}")
            continue
        expected_evidence = {
            "run_name": run_name,
    ...
    report = {
        ...
        "evidence_level": "revised_formal_single_seed_training",
    ...
    if errors:
        raise SystemExit(
            "Ranked-length training audit failed: " + " | ".join(errors)
        )
