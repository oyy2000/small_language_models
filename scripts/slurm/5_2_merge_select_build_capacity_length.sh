...
#SBATCH -J 5_2_capacity_length_build
#SBATCH -p a6000
#SBATCH -N 1
...
case "${STAGE}" in
  smoke) EXPECTED_PROBLEMS="${EXPECTED_PROBLEMS:-200}" ;;
  formal) EXPECTED_PROBLEMS="${EXPECTED_PROBLEMS:-2000}" ;;
...
python3 -u scripts/5_2_merge_select_capacity_length_traces.py \
  --config "${CONFIG}" \
  --input-glob "${STAGE_ROOT}/raw/*/shard_*.jsonl" \
  --output-dir "${STAGE_ROOT}/selected" \
  --stage "${STAGE}" \
...
echo "selection_and_dataset_build_complete stage=${STAGE} output=${STAGE_ROOT}"
